...
import { ActionModel, BS, EventModel }                 from '../models';
...
export async function mint_bridge_action(db_by: any, db_to: any, id: number, block: number, infos: any): Promise<void> {
    ...
    await BS.transaction(async (t: any) => {
        ...
        .then((): void => {
            Signale.success(`[evm-events][mint] by: ${db_by.attributes.address} to: ${db_to.attributes.address} id: ${id} block: ${block}`);
        })
        .catch((e: Error): void => {
            Signale.error(`[evm-events][mint] by: ${db_by.attributes.address} to: ${db_to.attributes.address} id: ${id} block: ${block}`);
            Signale.error(e);
